////////////////////////////////////////////////////////////////////
//DeRap: Produced from mikero's Dos Tools Dll version 4.17
//Sun Mar 20 02:09:34 2016 : Source 'file' date Sun Mar 20 02:09:34 2016
//http://dev-heaven.net/projects/list_files/mikero-pbodll
////////////////////////////////////////////////////////////////////

#define _ARMA_

class CfgPatches
{
	class VME_CLP_A3_FONTS
	{
		units[] = {};
		weapons[] = {};
		requiredVersion = 0.1;
		requiredAddons[] = {"A3_Data_F","A3_UiFonts_F"};
	};
};
class CfgFontFamilies
{
	class EtelkaMonospacePro
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK30"};
	};
	class EtelkaMonospaceProBold
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK-Bold20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK-Bold30"};
	};
	class EtelkaNarrowMediumPro
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK30"};
		spaceWidth = 0.9;
		spacing = 0.17;
	};
	class PuristaBold
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK-Bold20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK-Bold30"};
	};
	class PuristaLight
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK30"};
	};
	class PuristaMedium
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK30"};
	};
	class PuristaSemibold
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK-Bold20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK-Bold30"};
	};
	class RobotoCondensed
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK30"};
	};
	class RobotoCondensedBold
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK-Bold20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK-Bold30"};
	};
	class RobotoCondensedLight
	{
		fonts[] = {"\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK20","\vme_clp_a3_language_uifonts_f_data\fonts\NotoSansCJK30"};
	};
};